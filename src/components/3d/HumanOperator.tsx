...
export const HumanOperator = ({ id, rotation, isStanding, isInspection }: HumanOperatorProps) => {
    ...
    const styleSeed = (seed * 13) % 3; // 0, 1, 2 for hairstyle variation
    ...
    const chairMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: '#1e1e1e', roughness: 0.4 }), []);
    const metalMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: '#94a3b8', roughness: 0.3, metalness: 0.8 }), []);

    // PPE Materials
    const coatMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: '#bae6fd', roughness: 0.9 }), []); // Sky blue
    const ppeBlueMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: '#38bdf8', roughness: 0.9 }), []); // Surgical blue

    // In Machine3D, this component is placed INSIDE the machine's group.
    // That means `position={[0,0,0]}` is the exact center of the machine.
    // It also means the operator inherits the machine's rotation.
    // If we want the operator to sit *in front* of the machine, we offset +Z
    // (since machines face -Z natively). We also spin the operator 180 (Math.PI)
    // to face back at the machine.
    ...
    return (
        <group position={[0, 0, 0.55]} rotation={[0, Math.PI, 0]} scale={[0.8, 0.8, 0.8]}>
            ...
            {/* --- HUMAN BODY --- */}
            <group position={[0, isStanding ? 0.75 : 0.48, isStanding ? 0.1 : 0]}> {/* Sat or standing */}
                ...
                <group position={[0, 0.16 + torsoHeight + 0.2, 0.02]}>
                    ...
                    {/* Nose (for realism and face direction) */}
                    <mesh position={[0, -0.01, 0.13]}>
                        <sphereGeometry args={[0.02, 16, 16]} />
                        <primitive object={skinMaterial} />
                    </mesh>

                    {/* Surgical Cap (Pushed back off the face) */}
                    <mesh position={[0, 0.02, -0.04]} rotation={[-0.3, 0, 0]}>
                        <sphereGeometry args={[0.135, 32, 32, 0, Math.PI * 2, 0, Math.PI / 1.6]} />
                        <primitive object={ppeBlueMaterial} />
                    </mesh>
                </group>

                {/* Arms */}
                {/* Adjust arm positioning for inspection */}
                {[-1, 1].map((side) => {
                ...
                {[-1, 1].map((side) => (
                    <group key={side} position={[side * (torsoWidth / 2 - 0.08), 0.08, isStanding ? 0 : 0.1]}>
                        {isStanding ? (
                            ...
                            <>
                                {/* Upper Leg (horizontal along seat) */}
