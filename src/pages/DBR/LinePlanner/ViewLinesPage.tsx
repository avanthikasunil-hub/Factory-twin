...
const ViewLinesPage = () => {
  ...
  // Show DBR lines + legacy lines (without factoryId)
  const savedLines = useMemo(() => 
    allLines.filter(line => line.factoryId === 'dbr' || !line.factoryId), 
    [allLines]);
  ...
  const handleOpenLine = (id: string, lineNo: string) => {
    loadLine(id);
    navigate('/dbr/line-planner/planner');
    toast({
      title: "Line Loaded",
        ...
        <div className="max-w-[1600px] mx-auto w-full flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => navigate('/dbr/line-planner')}
