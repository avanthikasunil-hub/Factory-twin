...
const LinePlannerPage = () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  const stats = useMemo(() => {
    const finalOps = Array.isArray(operations) ? operations : [];
    const totalStyleSMV = currentLine?.totalSMV ||
      finalOps.reduce((sum, op) => sum + (op?.smv || 0), 0);
    ...
    return {
      ...
      totalStyleSMV,
      lineCapacity: 1800,
      bottleneckOpName: assemblyBottleneck // Root bottleneck for assembly summary
    };
